...
async function main() {
  ...
  // ---------- 33. ir_opps release order — "...release A, B, C if needed"
  {
    ...
    // than reuse a release that already went to someone else.
    {
      ...
      // so the second row's honest fallback just says the IR is full — never a
      // reuse of the same occupant's name, and never names anyone at all.
      ok(decisionsText.includes("New Eligible Beta (IR): IR full"), "the second player who couldn't be paired still gets an honest fallback, not a silently-reused release", decisionsText);
    }
    ...
    // plainly that nobody on the list is really on IR there.
    {
      ...
      const decisionsText = decisionsTextOf(out.blocks);
      ok(decisionsText === "Fallback New Guy (IR): IR full", "just says 'IR full', no names at all, once a release order is active — strict allow-list, not a silent fallback", decisionsText);
      ok(!/Real Occupant Here/.test(decisionsText) && !/Not Rostered Anywhere Near/.test(decisionsText), "neither the real unapproved occupant nor the release list itself is named anywhere in the output");
    }
    ...
    // names the protected player in their own release order.
    {
      ...
      ok(!/Protected Release Target/.test(decisionsTextOf(out.blocks)), "he's never named anywhere, even though the owner named him explicitly");
      ok(decisionsTextOf(out.blocks) === "Prio New Guy (IR): IR full", "with the only occupant protected, this is honestly reported as just 'IR full' — no names at all, not silently skipped", decisionsTextOf(out.blocks));
    }
  }

  // ---------- 34. ir_opps standing IR Release list — ONE single command
  // applies it automatically and silently; never names an unapproved
  // player, even in the honest "IR full" fallback. Collapsed back from an
  // earlier two-command design (bare report vs "...and drop") after real
  // use showed the bare command still naming players outside the owner's
  // list — the owner asked for one prompt, strict, no names ever.
  {
    const draftsOf = (blocks: Block[]) => blocks.filter((b): b is Extract<Block, { t: "drafts" }> => b.t === "drafts").flatMap((b) => b.drafts);
    const decisionsTextOf = (blocks: Block[]) => blocks.filter((b): b is Extract<Block, { t: "decisions" }> => b.t === "decisions").flatMap((b) => b.rows.flatMap((r) => r.items)).join(" | ");
    const rp = ["QB", "WR", "BN"];
    ...
    // (a) a bare "move all my IR eligible players to IR" with a standing
    // list configured DOES draft the real release+move pair — no "and
    // drop" phrasing needed, no inline clause needed.
    {
      const pm: PlayerMap = {
    ...
    // (b) an inline release clause on ONE command still wins over the
    // standing list, for that one command.
    {
      const pm: PlayerMap = {
    ...
    // (c) strict fallback: a league where the standing list doesn't apply
    // never falls back to naming some other real occupant — just "IR full",
    // no names at all, not even the list itself.
    {
      const pm: PlayerMap = {
      ...
      ok(decisionsText === "Strict New Guy (IR): IR full", "just says 'IR full' — no names at all, neither the real occupant nor the standing list itself", decisionsText);
    }

    // (d) an empty standing list (the ordinary case for an owner who
    // hasn't set one up) changes nothing: original unrestricted behavior.
    {
      const pm: PlayerMap = {
