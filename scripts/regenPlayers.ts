// Regenerates the curated player list (RankedPlayer in the DB) from real
// Sleeper data: top N per position (QB/RB/WR/TE) by season PPR point
// projection, tiered by quantile within each position's own pool. Run with
// `npm run regen-players`.
//
...
// signings) rather than hand-adding players one at a time.
//
...
async function main() {
  ...
  // Master order: tier major (top-to-bottom by quality), position minor,
  // points-descending within each — this also keeps each position's
  // entries in strictly descending point order across the whole list,
  // which is what makes posRank (computed from order alone) come out
  // correct.
  out.sort(
    (a, b) =>
      a.tier - b.tier ||
